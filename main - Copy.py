...
class GameSetup(BoxLayout):
    def __init__(self,**kwargs):
        ...
        normalButton = Button(text="NORMAL", size_hint = (0.5, 0.5))
        normalButton.bind(on_press = self.startNormal)

        hardButton = Button(text = "HARD", size_hint = (0.5, 0.5))
        hardButton.bind(on_press = self.startNormal)

        helpButton = Button(text = "HELP", size_hint = (0.5, 0.5))
        helpButton.bind(on_press = self.loadHelp)

        scoreButton = Button(text = "HIGH SCORES", size_hint = (0.5, 0.5))
        scoreButton.bind(on_press = self.loadScores)

        exitButton = Button(text = "EXIT", size_hint = (0.5, 0.5))
...
#These two steps are done just so that the dict is properly initialized.
    ...
    def begin(self,app,root):
		...
		with self.canvas:
			...
			print (self.snake1.center_x,self.snake1.center_y)
			Color(0,191,255)
